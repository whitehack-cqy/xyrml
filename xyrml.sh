...
============================================================
	  云免流控™-Web流控系统 云免服务器一键搭建
		Powered by www.xiaoyangren.net 2020
			All Rights Reserved

				by 云免流控™ 2020-02-18
		垃圾小洋人 烟雨如花破解 烟雨如花yyrh.me
============================================================';
http='http://';
host='oss.ymlk.top/';
vpnfile=b22655b1f6c57ae33676539ae247605901cc1f93/;
squser=auth_user;
css=errorpage.css;
dis=disconnect.s;
mp=mproxy;
...
#key='xiaoyangren.net';
key='yyrh.me';
jiankong_tcp='tcpjiankong.z';
jiankong_udp='udpjiankong.z';
webfile='xyr-web.z';
...
#    echo -n -e "请输入云免流控™官方网址 [\033[32m $key \033[0m] ："
    echo -n -e "请输入烟雨如花小站网址 [\033[32m $key \033[0m] ："
    read PASSWD
    readkey=$PASSWD
    if [[ ${readkey%%\ *} == $key ]]
    then
		...
		echo -e '\033[33m		Powered by www.xiaoyangren.net 2020\033[0m'
		echo -e '\033[33m		All Rights Reserved \033[0m'
		echo -e '\033[33m	官方网址：http://www.xiaoyangren.net/ \033[0m'
		echo -e '\033[33m	交流①群：438968654	交流②群：167212446  欢迎你的加入！\033[0m'
		echo -e '\033[33m=========================================================\033[0m'
...
echo
echo -e "是否生成流量APP？[y/n]（回车默认生成，集群负载中的副机无需生成）"
read llwsapp
...
iptables -t nat -A PREROUTING -p udp -m udp --dport 123 -j DNAT --to-destination $IP:53
...
iptables -t nat -A PREROUTING -p udp -m udp --dport 3128 -j DNAT --to-destination $IP:53
iptables -t nat -A PREROUTING -p udp -m udp --dport 3389 -j DNAT --to-destination $IP:53
iptables -t nat -A PREROUTING -p udp -m udp --dport 3848 -j DNAT --to-destination $IP:53
...
rpm -Uvh --force ${http}${host}${vpnfile}op-2.3.2-4.el7.x86_64.rpm
rm -rf ./op-2.3.2-4.el7.x86_64.rpm
...
wget -O easy-rsa.tar.gz ${http}${host}${vpnfile}${RSA}
tar -zxvf easy-rsa.tar.gz >/dev/null 2>&1
rm -rf easy-rsa.tar.gz
wget -O xyr.config ${http}${host}${vpnfile}xyr.c >/dev/null 2>&1
...
echo "正在安装HTTP转发模式..."
cd /etc/openvpn
wget -O mproxy ${http}${host}${vpnfile}${mp}
...
cd /root
wget -O res.zip ${http}${host}${vpnfile}res.z >/dev/null 2>&1
...
echo "请不要进行任何操作..."
cd /root
wget -O xyr-web.zip ${http}${host}${vpnfile}${webfile}
unzip -o xyr-web.zip >/dev/null 2>&1
...
wget -O tcpjiankong.zip ${http}${host}${vpnfile}${jiankong_tcp} >/dev/null 2>&1
unzip -o tcpjiankong.zip >/dev/null 2>&1
rm -rf tcpjiankong.zip
chmod 777 jiankong
chmod 777 sha

chmod 755 /home/wwwroot/default/udp >/dev/null 2>&1
cd /home/wwwroot/default/udp
wget -O udpjiankong.zip ${http}${host}${vpnfile}${jiankong_udp} >/dev/null 2>&1
unzip -o udpjiankong.zip >/dev/null 2>&1
...
cd ${web_path}
wget -O app_api.zip ${http}${host}${vpnfile}app_api.z && unzip -o app_api.zip >/dev/null 2>&1
rm app_api.zip
cd ${web_path}/app_api
...
EOF
echo "安装流量监控..."
wget -O disconnect.sh ${http}${host}${vpnfile}disconnected.s
...
yum install -y java
echo "下载APK包"
wget -O android.apk ${http}${host}${vpnfile}v5.app
echo "清理旧的目录"
rm -rf android
echo "分析APK"
wget -O apktool.jar ${http}${host}${vpnfile}apktool.j&&java -jar apktool.jar d android.apk
echo "批量替换"
...
echo "APK生成完毕"
wget -O autosign.zip ${http}${host}${vpnfile}autosign.z && unzip -o autosign.zip
...
mkdir /home/xianlu
cd /home/xianlu
wget -O line.zip ${http}${host}${vpnfile}line.z >/dev/null 2>&1
unzip -o line.zip >/dev/null 2>&1
...
wget -O backupsql.sh -P /home ${http}${host}${vpnfile}backupsql.s >/dev/null 2>&1
...
# echo -e '\033[33m                        Powered by xiaoyangren.net 2020\033[0m'
# echo -e '\033[33m                        All Rights Reserved\033[0m'
# echo -e '\033[33m                交流①群：438968654	交流②群：167212446  欢迎你的加入！\033[0m'
...
echo -e '\033[34m                垃圾小洋人流控 烟雨如花破解 烟雨如花yyrh.me\033[0m'
echo -e '\033[33m================☆☆========================================================\033[0m'
...
echo -e '\033[33m检测到烟雨如花已经破解这个垃圾，下面请回车开始破解 \033[0m'
echo
#echo  -n -e '\033[33m请输入授权密钥：\033[0m'
echo  -n -e '\033[33m请回车开始破解这个垃圾：\033[0m'
read card
#curl "http://oss.ymlk.top/status.php?url=$IP&km=$card&qq=$qie" >/dev/null 2>&1
echo
echo "正在验证授权..."
#kcard=`curl -s http://oss.ymlk.top/card.php?url=${IP}"&km="${card}"&qq="${qie}`;
kcard=true
if [[ "$kcard" == "false" ]] || [ "$kcard" == "" ]
then
echo -e '\033[33m==========================================================================\033[0m'
echo -e '\033[34m               未能通过验证 请确认您的密钥是否正确！\033[0m'
echo -e '\033[31m               温馨提示：\033[0m'
echo -e '\033[31m               为了您服务器的稳定和安全，请勿非法破解改程序\033[0m'
echo -e '\033[33m               正版密钥12元/1件\033[0m'
echo -e '\033[31m               密钥绑定IP可在同一IP下反复使用！\033[0m'
echo -e '\033[33m               官方网址：http://www.xiaoyangren.net/\033[0m'
echo -e '\033[31m               交流①群：438968654	交流②群：167212446	欢迎你的加入！\033[0m'
echo -e '\033[33m==========================================================================\033[0m'
exit 0;
else
if [[ "$kcard" == "true" ]]
...
echo -e "破解状态  $IP  [\033[32m  破解成功  \033[0m]";
#echo "授权码已成功绑定您的服务器IP，支持永久无限使用！";
echo "这个垃圾已经被破解，去奔放吧，By烟雨如花，博客yyrh.me";
echo "即将开始下一步安装..."
sleep 0.5
echo "请选择安装类型："
echo
...
wget -O disconnect.sh ${http}${host}${vpnfile}${dis}
chmod 0777 ./disconnect.sh >/dev/null 2>&1
